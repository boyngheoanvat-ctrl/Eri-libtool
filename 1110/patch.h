...
bool hex2bytes(char* bytes, unsigned char* buffer)
{
    ...
    for(int i=0; i<len; i++) {
        ...
        else if(_byte>='a' && _byte<='f') _byte -= 'a'-10;
        else if(_byte>='A' && _byte<='F') _byte -= 'A'-10;
        else return false;
...
NSString* Hook1110(char* machoPath, uint64_t vaddr, char* patch)
{
    ...
    if(newPath) {
        ...
        if(!macho) return [NSString stringWithFormat:@"Documents/hackkkkne/%s", machoPath];
    } else {
        macho = load_macho_data(path);
        if(!macho) return [NSString stringWithFormat:@".app/%s", machoPath];
    }
    ...
    while(true) {
        ...
        if(!macho) return @"add_hook_section error!";
    }
    
    if(cryptid != 0) return @"app is not decrypted!";
    if(!text_seg || !data_seg) return @"can not parse machO file!";
    ...
    if(patch && patch[0]) {
        ...
        if(!hex2bytes(patch, (uint8_t*)patch_bytes+vaddr%4)) return @"The bytes to patch are incorrect!";
    } else if(vaddr % 4) {
    ...
    for(int i=0; i<STATIC_HOOK_CODEPAGE_SIZE/sizeof(StaticInlineHookBlock); i++)
    {
        if(hookBlock[i].hook_vaddr==funcRVA) {
            if(patch && patch[0] && hookBlock[i].patch_hash!=calc_patch_hash(vaddr, patch))
                return @"The bytes to patch have changed!";
            return @"The offset to hook is already patched!";
        }
        if(funcRVA>hookBlock[i].hook_vaddr && (funcRVA < (hookBlock[i].hook_vaddr+hookBlock[i].hook_size) || funcRVA < (hookBlock[i].hook_vaddr+hookBlock[i].patch_size))) {
    ...
    if(!hookBlockRVA) return @"HOOK count full!";
    
    if(!dobby_static_inline_hook(hookBlock, hookBlockRVA, funcRVA, funcData, targetRVA, targetData, InstrumentBridgeRVA, patch_bytes, patch_size)) {
    ...
    if(![macho writeToFile:savePath atomically:NO]) return @"can not write to file!";
...
void* find_module_by_path(char* machoPath)
{
    ...
    for(int i=0; i< _dyld_image_count(); i++) {
        ...
        if([path isEqualToString:[NSString stringWithUTF8String:fpath]]) return baseaddr;
    }
    return NULL;
}
...
StaticInlineHookBlock* find_hook_block(void* base, uint64_t vaddr)
{
    ...
    for(int i=0; i<STATIC_HOOK_CODEPAGE_SIZE/sizeof(StaticInlineHookBlock); i++) {
        if(hookBlock[i].hook_vaddr == (uint64_t)vaddr) return &hookBlock[i];
    }
    return NULL;
}
...
BOOL ActiveCodePatch(char* machoPath, uint64_t vaddr, char* patch)
{
    ...
    if(!hookBlock) return NO;
    if(hookBlock->patch_hash != calc_patch_hash(vaddr, patch)) return NO;
